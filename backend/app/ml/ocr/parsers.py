...
class AadhaarParser:
    ...
    def extract(self, text: str) -> dict:
        ...
        # Pass 3: fuzzy fallback — tolerate OCR letter/digit confusion
        # (O/0, I/1, l/1, B/8, S/5) when the strict passes above found nothing.
        if chosen_number is None:
            ...
            if fuzzy_match:
                ...
                if (
                    len(cleaned_digits) == 12
                    and cleaned_digits.isdigit()
                    and cleaned_digits[0] not in ('0', '1')
                    and not (1900 <= int(cleaned_digits[:4]) <= 2099)
                ):
                    chosen_number = cleaned_digits
                    logger.debug(
                        f"AadhaarParser: fuzzy fallback candidate ending in ...{cleaned_digits[-4:]} "
                        f"(verhoeff={_verhoeff_ok(cleaned_digits)})"
                    )
