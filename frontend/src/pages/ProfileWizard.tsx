...
function mapExtractionToProfile(
  ...
  if (backendDocType === 'aadhar') {
    ...
    // District from 7/12 only fills the profile if it is currently empty
    ...
    satbaraExtraction = {
      ...
      village:        fieldValue(fields, 'village') || undefined,
      ...
      taluka:         fieldValue(fields, 'taluka') || undefined,
      gat_number:     fieldValue(fields, 'gat_number') || undefined,
      filledFields,
    };

    // Conditionally fill district — only if blank
    if (districtVal) {
      // Will be applied conditionally in the handler based on current profile state
    ...
    try {
      ...
      setProfile((prev) => {
        ...
        for (const [key, value] of Object.entries(profileUpdates)) {
          ...
          // For district from 7/12, only fill if currently blank
          if (k === 'district' && prev.district) continue;
